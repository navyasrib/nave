...
operations.handleIfCondition = function(condition, action) {
    if (condition.evaluate().equals(new classes.Bool(true)))
        return action.evaluate();
};

operations.handleElseCondition = function(condition, action){
  if (condition.evaluate().equals(new classes.Bool(false)))
      return action.evaluate();
}
